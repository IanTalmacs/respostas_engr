...
function renderPlayers(list){
  ...
  for(const p of list){
    ...
    el.innerHTML=`<div>${p.name}</div><div>${p.score||0}</div>`
...
function renderHand(h){
  ...
  for(const c of h){
    ...
    b.className='cardWhite'
    b.textContent=c
    b.onclick=()=>{if(state.started&&state.judgeId!==playerId&&!state.roundActive){socket.emit('submitCard',{playerId,card:c})}}
...
function renderScores(list){
  ...
  for(const p of list){
    ...
    s.className='scoreItem'
    s.textContent=`${p.name}: ${p.score||0}`
...
startRoundBtn.onclick=()=>{socket.emit('startRound',{playerId})}
...
socket.on('judgeChoices',entries=>{
  ...
  for(const e of entries){
    ...
    d.className='choice'
    d.innerHTML=`<input type="radio" name="choiceRadio" value="${e.pid}"> ${e.card}`
